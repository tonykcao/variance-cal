...
export async function POST(request: NextRequest) {
  try {
    ...
    if (endMinutes !== 0 && endMinutes !== 30) {
      return NextResponse.json(
        { error: "End time must be aligned to 30-minute boundaries (:00 or :30)" },
        { status: 400 }
      )
    }

    // Use the already-aligned times
    const startUtcSnapped = startUtc
    const endUtcSnapped = endUtc
    ...
    const durationMs = endUtcSnapped.getTime() - startUtcSnapped.getTime()
    ...
    if (input.attendees && input.attendees.length > 0) {
      ...
      for (const email of newEmails) {
        ...
        console.log(
          `[POST /api/bookings] Created new user for attendee: ${email} (${nameFromEmail})`
        )
      }
