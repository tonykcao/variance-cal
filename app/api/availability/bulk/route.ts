...
export async function POST(request: NextRequest) {
  try {
    ...
    const { includeRoomDetails = true, includeDateSummary = true, maxResults = 100 } = options
        ...
        try {
              ...
              return {
                ...
                summary: includeDateSummary
                  ? {
                      ...
                      totalOccupiedSlots,
                      utilization:
                        totalAvailableSlots + totalOccupiedSlots > 0
                          ? Math.round(
                              (totalOccupiedSlots / (totalAvailableSlots + totalOccupiedSlots)) *
                                100
                            )
                          : 0,
                    }
          ...
          const overallSummary = {
            ...
            totalAvailableSlots: dateRangeResults.reduce(
              (sum, range) => sum + (range.summary?.totalAvailableSlots || 0),
              0
            ),
            totalOccupiedSlots: dateRangeResults.reduce(
              (sum, range) => sum + (range.summary?.totalOccupiedSlots || 0),
              0
            ),
          }
    ...
    const overallStats = {
      ...
      totalAvailableSlots: results.reduce(
        (sum, r) => sum + (r.summary?.totalAvailableSlots || 0),
        0
      ),
    ...
    exampleRequest: {
      requests: [
        ...
        },
        {
          ...
          dateRanges: [{ from: "2025-09-26", to: "2025-09-28" }],
