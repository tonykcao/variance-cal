...
describe("Availability API Integration Tests", () => {
  ...
  beforeAll(async () => {
    ...
    // Set up necessary test data for availability queries
    // Create a test site and room if they don't exist
  ...
  describe("Basic Availability Queries", () => {
    ...
    it("should filter by sites correctly", async () => {
      // Get any site from the database
      const site = await prisma.site.findFirst()

      if (!site) {
        // Skip test if no sites in database
        console.log("No sites in database, skipping test")
        return
      }
      ...
      // All rooms should be from the selected site
      data.rooms.forEach((room: any) => {
        expect(room.siteId).toBe(site.id)
        expect(room.siteName).toBe(site.name)
      })
    })
  ...
  describe("Availability with Bookings", () => {
    ...
    it("should handle overlapping bookings correctly", async () => {
      ...
      const users = [existingUser!, secondUser]
      const today = format(new Date(), "yyyy-MM-dd")
          ...
          data: {
            ...
            slots: {
              create: [
                {
                  roomId: room!.id,
                  slotStartUtc: localToUtc(`${today}T09:00`, room!.site.timezone),
                },
                {
                  roomId: room!.id,
                  slotStartUtc: localToUtc(`${today}T09:30`, room!.site.timezone),
                },
              ],
            },
          },
          ...
          data: {
            ...
            slots: {
              create: [
                {
                  roomId: room!.id,
                  slotStartUtc: localToUtc(`${today}T11:00`, room!.site.timezone),
                },
                {
                  roomId: room!.id,
                  slotStartUtc: localToUtc(`${today}T11:30`, room!.site.timezone),
                },
              ],
            },
          },
