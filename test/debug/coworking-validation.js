  ...
  await testAPI('/api/availability?from=2025-09-25&to=2025-09-25', (data) => {
    if (data.rooms && data.rooms.length > 0) {
      ...
      if (available.length > 0) {
        ...
        console.log(`   ✓ ${room.siteName} rooms: Available ${first.toLocaleString('en-US', {timeZone: room.timezone, timeStyle: 'short'})} - ${last.toLocaleString('en-US', {timeZone: room.timezone, timeStyle: 'short'})} local time`);
        return true;
      }
    }
    return false;
